<template>
    <div class="seller-order-item">
	    <div class="order-item-head" @click="redirectToUserIndex">
	    	{{item.buyerName || '匿名用户'}} <i class="iconfont icon-right"></i>
	    ...
	    <div class="order-item-footer" ref="actionsBox">
	    	...
	    	<div v-else>
	    		<span v-if="item.status == 4" @click="closeOrder">关闭交易</span>
		    	<span @click="updatePrice" v-if="item.status == 4">修改价格</span>
		    	<span v-if="closeStatus" @click="deleteOrder">删除订单</span>

		    	<span class="color" @click="logisticsInfo" v-if="item.status == 0">填写物流信息</span>
	    	</div>
	    </div>
    </div>
</template>
        ...
        methods: {
            ...
            redirectToUserIndex() {
                ...
                window.mainView.router.load({
                    url: `views/otherIndex.html?currentUserId=${vm.item.buyerUserId}`
                })
            },
			// 跳转到订单详情
			redirectToOrderInfo() {
				...
				window.mainView && window.mainView.router.load({
				    url: `views/transactionProcess/orderDetails.html?orderNo=${orderNo}`
				});
			},
			// 修改价格
			updatePrice(){
				const {orderNo, payTotalMoney} = this.item
				window.mainView && window.mainView.router.load({
			...
			logisticsInfo(){
				const {orderNo} = this.item
				window.mainView && window.mainView.router.load({
				    url: `views/transactionProcess/logisticsInfo.html?orderNo=${orderNo}`
				});
			},
			// 查看帮助详情
            viewHelpInfo() {
                            ...
                            onClick: () => {
                                ...
                                TransactionProcessModel.closeOrder({
				                    ...
				                    if (1 === code){
				                        ...
				                        nativeEvent['nativeToast'](1, '交易已关闭~');
			                            return;
				                    }
				                    f7.alert(message, '提示');
				                });
...
<style lang="less">
	...
	.seller-order-item{
		...
		background: @bg-fff;
		.order-item-head{
			background-image: url('../../build/img/ic_buyer.png');
