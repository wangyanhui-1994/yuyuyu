...
function myBuyOrderInit (f7, view, page){
    /**
     * 获取当前页面
     */
    const pages = $$('.view-main .pages>.page-my-buy-order');
    ...
    let navBuyName = $nav.find('.buy-name');
    ...
    const getOrderList = () => {
        TransactionProcessModel.getMyBuyOrderList({
            pageNo,
            pageSize,
            type
        }, (res) => {
     ...
     * @author [candy]
     */
    const getToBePaidBuy = () => {
        TransactionProcessModel.getToBePaidBuy((res) => {
            const { code, data, message } = res;
            if (1 != code){
                console.log(message);
                return;
            }
            vueData.isHasToBePayOrder = data;
        });
    };
